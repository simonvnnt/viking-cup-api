...
class KpiApiController extends AbstractController
{
    ...
    #[Route('/incomes/tickets', methods: ['GET'])]
    public function getIncomesTicketsKpi(
        KpiBusiness $kpiBusiness,
        #[MapQueryParameter] ?int $eventId = null,
        #[MapQueryParameter] ?int $roundId = null,
    ): JsonResponse
    {
        $ticketsKpi = $kpiBusiness->getIncomesTicketsKpi($eventId, $roundId);

        return $this->json($ticketsKpi, Response::HTTP_OK, [], ['groups' => ['round', 'roundEvent', 'event', 'category']]);
    }

    #[Route('/accounting/summarise', methods: ['GET'])]
    public function getAccountingSummariseKpi(
        KpiBusiness $kpiBusiness,
        #[MapQueryParameter] ?int $eventId = null,
        #[MapQueryParameter] ?int $roundId = null,
    ): JsonResponse
    {
        $accountingSummarise = $kpiBusiness->getAccountingSummariseKpi($eventId, $roundId);

        return $this->json($accountingSummarise);
    }
}